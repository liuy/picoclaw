...
func (al *AgentLoop) runTurn(ctx context.Context, ts *turnState) (turnResult, error) {
	...
	if !ts.opts.NoHistory {
		...
		// Let ContextManager assemble budget-aware history.
	...
	for ts.currentIteration() < ts.agent.MaxIterations || len(pendingMessages) > 0 || func() bool {
		...
		if allResponsesHandled {
			...
			if ts.opts.EnableSummary {
				al.contextManager.Compact(turnCtx, &CompactRequest{SessionKey: ts.sessionKey, History: ts.agent.Sessions.GetHistory(ts.sessionKey), Reason: ContextCompressReasonSummarize})
			}
