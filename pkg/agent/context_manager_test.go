...
func TestRegisterContextManager_Success(t *testing.T) {
	...
	factory := func(cfg json.RawMessage) (ContextManager, error) {
...
func TestRegisterContextManager_EmptyName(t *testing.T) {
	...
	err := RegisterContextManager("", func(cfg json.RawMessage) (ContextManager, error) {
		return &noopContextManager{}, nil
	})
	if err == nil {
...
func TestLegacyAssemble_Passthrough(t *testing.T) {
	...
	if len(resp.Messages) != len(history) {
		t.Fatalf("expected %d messages, got %d", len(history), len(resp.Messages))
	}
...
func TestLegacyAssemble_EmptyHistory(t *testing.T) {
	...
	if len(resp.Messages) != 0 {
		t.Fatalf("expected empty messages, got %d", len(resp.Messages))
	}
}
